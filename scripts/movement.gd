...
@export var double_jump_strength := 1200.0 # a weaker jump strength for double jumps
...
func _physics_process(delta: float) -> void:
	...
	var is_double_jumping := Input.is_action_just_pressed("jump") and is_on_floor() and coyote_time.is_stopped()
